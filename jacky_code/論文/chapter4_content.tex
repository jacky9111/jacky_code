...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.4\columnwidth]{ch4_beam_on_per_satellite}
  \caption{Illustrative satellite-fixed beam layout with 16 downlink beams indexed from north to south.}
  \label{fig:ch4_beam_on_per_satellite}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\columnwidth]{ch4_OneWeb-like}
  \caption{Illustrative OneWeb-like local satellite geometry across three neighboring orbital planes. Each satellite employs the 16-beam layout.}
  \label{fig:ch4_oneweb_like}
\end{figure}

To explain the beam role identification and beam reassociation relations introduced later, this chapter uses a simplified OneWeb-like multi-beam scenario~\cite{oneweb2016fcc} for illustration. As shown in Fig.~\ref{fig:ch4_beam_on_per_satellite}, each satellite employs 16 satellite-fixed downlink beams, which are indexed from beam 1 to beam 16 according to their ground footprints from north to south. All satellites use the same beam layout and indexing rule, so each beam can be uniquely identified by its satellite and beam index.

As shown in Fig.~\ref{fig:ch4_oneweb_like}, a local OneWeb-like satellite geometry across three neighboring orbital planes, denoted by P1, P2, and P3, is used to illustrate the multi-satellite coverage relationship. Satellites S1 to S9 represent satellites at different orbital positions, while the GS is located near the equator. Each satellite employs the same 16-beam layout shown in Fig.~\ref{fig:ch4_beam_on_per_satellite}. Because the service regions of neighboring satellites partially overlap, the same ground area may be covered by beams from different satellites. These overlapping coverage relations provide alternative serving-beam candidates for user reassociation and motivate the proposed EABR framework.
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \textbf{EPFD-Constrained Power Allocation for Helper Recovery Beams:} Allocate the available helper satellite power to helper recovery beams according to the priority-weighted recovery demand, subject to beam power and aggregate EPFD constraints.
...
After the critical satellites, helper satellites, and beam roles are identified, the system constructs the candidate beam relation graphs according to the beam shutdown results and beam footprint overlap in each time slot. Fig.~\ref{fig:ch4_oneweb_like_geometry} shows the critical satellites, helper satellites, and closed-beam results in the illustrative scenario. If a critical safe beam overlaps with a helper release beam candidate, the corresponding beam pair is recorded in the safe-release candidate graph. If a critical closed beam overlaps with a helper recovery beam candidate, the corresponding beam pair is recorded in the closed-recovery candidate graph.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{{ch4_OneWeb-like local geometry}}
  \caption{Illustrative beam-role identification result in the OneWeb-like local geometry. Satellites S2, S5, and S8 are critical satellites, while S1, S3, S4, S6, S7, and S9 are helper satellites. The hatched regions indicate closed beams on the critical satellites.}
...
\subsection{EPFD-Constrained Power Allocation for Helper Recovery Beams}

After SBR is completed, the transmit power released by helper release beams is added back to the available power budget of the corresponding helper satellite. This available power budget consists of the residual satellite power remaining after the default beam-power allocation and the transmit power released through SBR. The system then allocates the available power to helper recovery beams on the same helper satellite for serving closed-beam users.
...
\For{each time slot $t$}
    ...
    \EndWhile
    \State \textbf{// EPFD-Constrained Power Allocation for Helper Recovery Beams}
