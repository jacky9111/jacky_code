...
\end{equation*}
where $L_{s,u}^{\mathrm{fs}}(t)$ is the free-space path loss~\cite{itur_p525} and $L_{s,u}^{\mathrm{gas}}(t)$ is the atmospheric gaseous loss~\cite{itur_p676}. The free-space path loss is given by
\begin{equation*}
...
The LEO intra-satellite interference comes from co-channel beams on the same satellite~\cite{cocco2014cochannel}. Let $\mathcal{B}_s^{\mathrm{co}}(b)$ denote the set of beams on satellite $s$ that use the same downlink frequency resource as the serving beam $b$, excluding beam $b$ itself. The intra-satellite interference received by user $u$ is given by
\begin{equation*}
...
After obtaining the user SINR, the achievable downlink capacity is modeled by the Shannon capacity formula~\cite{shannon1948}, which is commonly used in wireless and satellite communication systems. Let $B$ denote the downlink bandwidth of a beam. If user $u$ is served by beam $b$ of satellite $s$ at time slot $t$, the achievable downlink capacity of user $u$ is given by
\begin{equation*}
...
Let $D_u$ denote the service demand of user $u$. Following the demand satisfaction concept used in~\cite{jalali2023joint}, the satisfaction of user $u$ is calculated by comparing the actual service rate with the user demand and is defined as
\begin{equation}
...
\end{equation}
\begin{equation}
\sum_{b\in\mathcal{B}_s} p_{s,b}(t)\le P_{s,tot}^{\max}.
